// Vendored from @remix-run/v1-route-convention@0.1.4 (MIT, Remix Software Inc).
//
// That package is frozen and imports `@remix-run/dev/dist/config/routes` — a
// deep internal path into a package this app no longer installs. Its only
// runtime need from Remix was `createRouteId`, four lines, inlined below; the
// three types it imported are structural and are declared here too. Vendoring
// it is what lets the app keep its v1 nested-folder route layout with no
// @remix-run dependency at all. See app/routes.ts for the call site.
...
// Inlined from @remix-run/dev/dist/config/routes.
